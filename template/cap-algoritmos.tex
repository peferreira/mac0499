...
\paragraph*{}
No caso da IFT é criado um grafo a nível de pixels, a partir de um pixel $t$ a IFT se desloca através de $t$ para seus pixels adjacentes no grafo gerado para a imagem. No caso do grafo da imagem segmentada em superpixels, seja $s$ um superpixel, são criados arcos entre todos superpixels vizinhos de $s$, e os pesos dos arcos são dados pela norma do vetor diferença da cor média dos superpixels no espaço de cor do Lab.
\subsubsection{Caminhos no grafo da imagem}
Dado um grafo da imagem, temos que um caminho simples $\pi_t$ nesse grafo é uma sequência de pixels ou regiões, dependendo do tipo de segmentação da imagem. Todos os caminhos considerados nesse trabalho são caminhos simples, isto é, caminhos que passam por cada vértice no máximo uma vez. Assim, usamos a notação $\pi_t=\langle t \rangle$ para indicar um caminho composto por um único vértice, e $\pi_t=\pi_s.\langle s,t \rangle$ para indicar a extensão de um caminho $\pi_s$, com término em s, por uma aresta $(s,t)$ presente no grafo da imagem.
...
A função de custo utilizada nesse trabalho é a $f_{max}$, que é uma função suave, definida por:
...
A transformada imagem-floresta para um grafo $G=(\mathcal{I},I)$, e para a função de custo $f_{max}$, associa um caminho ótimo $\pi_t$ para cada pixel ou superpixel $t \in \mathcal{I}$. No entanto, $f$ deve ser suave, caso contrário os caminhos gerados podem não ser ótimos \citet{ift:ift_teoria}.
