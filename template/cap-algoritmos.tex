...
A abordagem utilizada para a IFT nesse trabalho é baseada em um grafo criado para a imagem. Como visto  em \citet{ift:ift_teoria}, temos que uma imagem \textbf{I} pode ser vista como uma tupla  $(\mathcal{I}, I)$  consistindo de um conjunto finito $\mathcal{I}$ composto por pixels (pontos no $\mathbb{Z}^{2}$)  e uma função $I$ que mapeia cada pixel $t$ em $\mathcal{I}$ algum valor $I(t)$ em algum espaço arbitrário.
\paragraph*{}
No caso da IFT a nível de pixels, a IFT percorre a imagem através de pixels adjacentes do grafo gerado para a imagem, e no caso deste trabalho serão todos os pixels que são adjacentes ao pixel escolhido. No caso da imagem segmentada em superpixels serão criados arcos entre superpixels vizinhos.
...
Será assumida uma função $f$ que associa a cada caminho $\pi$ um custo $f(\pi)$, em algum conjunto totalmente ordenado de valores de custo. É comum que o custo do caminho esteja diretamente ligado a propriedades da imagem, tais como cor, gradiente, e posição do pixel ou região ao longo do caminho.

A função de custo utilizada nesse trabalho para a IFT\_SLIC \citet{ift_slic:sp_iftslic} é a $f_{max}$, definida por:
