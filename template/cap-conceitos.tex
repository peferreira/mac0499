...
\paragraph*{}
Esses usuários tentam imitar um usuário com conhecimentos avançados em segmentação de imagens. O robô  dar prioridade a elementos da imagem (pixels ou superpixels) que estejam próximos e possuam caraciterísticas parecidas mas que tenham sido atribuidos a tipos diferentes (objeto ou fundo) pela segmentação da imagem. A ideia geral é que superpixels adjacentes que sejam similares mas possuam valores diferentes no gabarito indicam uma grande possibilidade de erro no nosso método de segmentação. 
\paragraph*{}

O usu da iteração sobre resultados gera uma melhora incremental nos resultados, e o método de segmentação utilizado e da esperteza do usuário influenciam no número de iterações necessárias. Um usuário robô garante um padrão nas decisões tomadas, que são baseadas em seu algoritmo, sendo assim podem gerar resultados mais confiáveis que um usuário real.
