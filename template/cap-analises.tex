...
\item ift\_sp.cpp implementação da IFT sobre superpixels utilizando uma implementação do SLIC.
\item ift.cpp implementação da IFT sobre pixels.
\item eroeval.cpp realiza chamada para iftsp e ift para varios raios de erosão da imagem.

\item Também foi utilizada a biblioteca externa gft, onde foi utilizada implementações de erosão de imagem, leitura e escrita de imagem.
\end{itemize}
...
\subsubsection{900 e 1600 superixels} Ambas obtiveram desempenho bem simular, e mostram que quando maior o número de superpixels, de acordo com esse experimento, pior foi a acurácia para raios pequenos de erosão, sementes que dão uma boa noção do objetvo verdadeiro.
